...
public class SystemConfig {
	...
	private final Map<Integer,Integer> plusScore;
	...
	public SystemConfig(Element system){
		...
		for(Element rect : rects){
			...
			//把Point对象数组放放到typeConfig中
			typeConfig.add(points);
		}
		//获得连消加分配置
		this.plusScore = new HashMap<Integer,Integer>();
		List<Element> plusScoreCfg = system.elements("plusScore");
		for (Element cfg : plusScoreCfg) {
			int rm = Integer.parseInt(cfg.attributeValue("rm"));
			int score = Integer.parseInt(cfg.attributeValue("score"));
			this.plusScore.put(rm, score);
		}
	}
	...
	public Map<Integer, Integer> getPlusScore() {
		return plusScore;
	}
